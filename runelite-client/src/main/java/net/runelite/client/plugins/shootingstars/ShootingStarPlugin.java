...
public class ShootingStarPlugin extends Plugin
{
	...
	@Inject
	private ShootingStarOverlayPanel overlayPanel;
	...
	private ShootingStarLocation lastLoc;
	...
	@Getter
	@Setter
	private List<ShootingStarData> starData = null;
	...
	@Provides
	ShootingStarConfig getConfig(ConfigManager configManager)
	{
	...
	public void onConfigChanged(ConfigChanged event)
	{
		switch (event.getKey())
		{
			case ShootingStarConfig.SHOOTING_STAR_POST_ENDPOINT_KEYNAME:
				shootingStarPostEndpoint = config.shootingStarPostEndpointConfig();
				break;
			case ShootingStarConfig.SHOOTING_STAR_GET_ENDPOINT_KEYNAME:
				shootingStarGetEndpoint = config.shootingStarGetEndpointConfig();
				break;
			default:
				break;
	...
	private void recordEvent(ShootingStarLocation loc, int world, int minTime, int maxTime)
	{
		long currentTime = Instant.now().toEpochMilli();
		long lminTime = currentTime / 1000 + (minTime * 60);
		long lmaxTime = currentTime / 1000 + (maxTime * 60);
		manager.storeEvent(new ShootingStarData(loc, world, lminTime, lmaxTime));
		lastWorld = world;
		lastLoc = loc;
	}
	...
	public void onGameTick(GameTick gameTick)
	{
		...
		if (chatbox != null)
		{
			...
			ShootingStarLocation loc = ShootingStarLocation.determineLocation(chatbox.getText());
